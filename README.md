# 🏛️ 大学生脑内常任委员议会 (Overthink Council)
...
你是否也曾在深夜面临这些致命纠结：
- *“明天早八高数课，舍友非拉我五排打瓦，去不去？”*
- *“暗恋对象凌晨一点发来一句'睡了吗'，该秒回还是欲擒故纵？”*
- *“月末生活费只剩 200 块，同学喊吃海底捞去不去？”*
...
### 1. 🎭 十一脑内常任委员全席巡礼
- **GPA 委员（椎名学委）**：卷王督察，绩点生命线守护者，张口闭口挂科率与重修费。
- **睡眠委员（悠悠）**：褪黑素保卫部，怨气极重、一票否决权常客，毒舌警告脑细胞凋亡。
- **快乐委员（乐天）**：精神内啡肽总局，及时行乐代表，坚信“高数可以补考，快乐只有今晚”。
- **钱包委员（钱真真）**：恩格尔系数守门人，抠门清醒，时刻核算隐形财务成本。
- **社交体面委员（苏语）**：情商与人情世故专员，极度在意朋友圈风评与人设崩塌。
- **傲骨尊严委员（凌傲）**：底线捍卫者，宁可玉碎不求妥协，二审反水掀桌专家。
- **未来推演司（陆远）**：终局演算者，以五年十年尺度审视沉没成本。
- **纯爱诗人、野心膨胀、道德法官、躯体维稳** 等 11 位特色人格全员集结！
...
- **执行细则与惩罚条款**：由获胜派别主导，附带“私自熬夜扣发次日多巴胺”等搞笑强制罚则。
- **防伪鲜红公章与印信**：带微倾斜仿真公章、支持一键复制公文文本与打印归档。

### 4. ⚖️ 二审紧急特别行政抗辩 (Emergency Appeal)
- 判决太离谱？当事人可提出重大不可抗力证据掀桌上诉！
- 触发全屏警报，急召特派委员（如尊严委员·凌傲）破壁介入，当场撕毁原裁决并颁布终审特别修正案。

### 5. 🧬 脑内神经元生态校准仪式 (Mind Calibration)
...
```
[ 用户端 UI (Next.js 16 + React 19) ]
   ├── 极简单对话框首页 (Whitespace-nowrap 响应式排印)
   ├── 议会激辩直播室 (逆转裁判特效 + 当庭抗辩输入条)
   ├── 投票表决厅 (戏剧性反水票 + 民主记名投票)
...
[ 后端服务层 (Next.js App Router API Routes) ]
   ...
   ├── POST /api/council/appeal       -> 二审急召新委员翻案
   ├── GET  /api/council/health       -> 系统健康与安全遥测
   └── POST /api/council              -> 通用统一网关
         │
         ▼
...
1. 登录 [Zeabur](https://zeabur.com/) 或 [Railway](https://railway.app/)。
2. 创建新项目并绑定你的 GitHub 仓库。
3. 平台会自动识别 Next.js 并完成编译与端口映射。
4. 在后台环境变量配置 `GEMINI_API_KEY`（如需），绑定自定义域名即可访问。

### 方案三：自建 Linux 云服务器部署 (Docker / Node.js)
...
# 2. 使用 PM2 守护进程常驻运行
npm install -g pm2
pm2 start npm --name "overthink-council" -- start -- -p 3000

# 3. 使用 Nginx 进行反向代理与 SSL 证书配置 (可选)
```
...
- **框架**：[Next.js 16.3.4](https://nextjs.org/) (App Router, Turbopack)
- **UI 库**：[React 19.2.8](https://react.dev/)
- **语言**：[TypeScript 5.0](https://www.typescriptlang.org/)
- **样式方案**：[Tailwind CSS 4.0](https://tailwindcss.com/) + 纯净 Vanilla CSS 玻璃拟态特效
